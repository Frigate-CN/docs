  ...
  <div class="home-section-content">
    ...
    <div class="block-item">
      ...
      <div class="block-content">
        ...
        <p>Frigate通过本地AI智能分析，自动过滤摄像头因阴影变化和风吹草动产生的误报，让你专注于真正重要的目标检测。搭载AI加速器时，系统每秒可处理100+个目标检测，精准识别人员、车辆等关键对象，确保监控视频的每一帧都得到有效分析，重要画面一帧不漏。</p>
      </div>
    </div>
    <div class="block-item">
      <div class="block-preview">
...
.underline:after {
    border-left: 1.5em solid #3c3d40;
    display: block;
    content: "";
    height: 2px;
    margin-top: .5em;
...
.home-section-content .block-item {
  display: flex;
  align-items: center;
  gap: 20px;
  width: 100%;
...
/* 奇数项：内容在左，预览在右 */
.block-item:nth-child(odd) .block-content {
  order: 1;
  margin-right: 4em;
}
...
/* 偶数项：内容在右，预览在左 */
.block-item:nth-child(even) .block-content {
  order: 2;
  margin-left: 4em;
}

.block-item:nth-child(even) .block-preview {
  order: 1;
}

.block-preview img {
  -webkit-filter: drop-shadow(2px 2px .25rem #2b2b2b);
  filter: drop-shadow(2px 2px .25rem #2b2b2b);
}
...
@media (max-width: 768px) {
  ...
  .block-preview {
    width: 100% !important;
    order: unset !important; /* 取消 order 控制，全部按 HTML 书写顺序排列 */
  }
