...
同时应避免选择WiFi摄像头，其视频流稳定性较差，易导致连接中断或视频数据丢失。尤其是在有多路摄像头的情况下，传输稳定性会受到极大干扰，甚至会影响到你整个WiFi其他设备的使用。（国内相关讨论，需要使用代理访问：[https://www.v2ex.com/t/1126166](https://www.v2ex.com/t/1126166)）
...
| -------------------- | -------------------------- | ------------------------- | ------------------------- | -------------------------------------- |
| Intel i3 6100T       | 15 - 35 ms                 |                           |                           | 只能运行一个检测器                       |
| Intel i5 6500        | ~ 15 ms                    |                           |                           |                                        |
...
| Intel i7 12650H      | ~ 15 ms                    | 320: ~ 20 ms 640: ~ 42 ms | 336: 50 ms                | 只能运行一个检测器                       |
| Intel N100           | ~ 15 ms                    | 320: ~ 25 ms              |                           |                                        |
