...
小米摄像头可以使用第三方应用 [micam](https://github.com/miiot/micam) 将 RTSP 流转发出来给 Frigate 接入。本文不提供 micam 的使用教程。
...
如果条件允许的话，应**尽可能避免或谨慎选择** WiFi 摄像头，其视频流稳定性较差，易导致连接中断或视频数据丢失。尤其是在有多路摄像头的情况下，传输稳定性会受到极大干扰，甚至会影响到你整个 WiFi 其他设备的使用。（中文相关讨论，需要使用代理访问：[https://www.v2ex.com/t/1126166](https://www.v2ex.com/t/1126166)）。

同时，目前市面上大部分的 WiFi 摄像头**并不是为长时间稳定推流**而设计的。默认情况下，它们仅将视频保存至存储卡亦或者云存储中，对稳定性要求并不高；但 Frigate 非常依赖实时处理视频流，对稳定性的要求极高。并且在连续长时间的获取实时视频流的情况下，摄像头的温度将会非常高，很容易影响连接稳定性。
...
- [Hailo](#hailo-8): Hailo8 和 Hailo8L AI 加速模块有 M.2 接口版本及树莓派 HAT 扩展版本。Hailo8l 参考价约为 **600 元**。
...
- [Google Coral EdgeTPU](#google-coral-tpu): Google Coral EdgeTPU 有 USB/M.2 两种版本。M.2 版参考价约为 **250 元**。
  - [主要支持 ssdlite 和 mobilenet 模型](/configuration/object_detectors#edge-tpu-detector)

- <Badge text="社区支持" type="warning" /> [MemryX](#memryx-mx3): MX3 M.2 加速模块采用 M.2 格式，可与广泛的设备兼容。
  - [支持多种模型架构](/configuration/object_detectors#memryx-mx3)
  - 最适合运行 tiny、small 或 medium 尺寸的模型
...
| 名称           | MobileNetV2 推理时间 | YOLOv9                                            | YOLO-NAS 推理时间         | RF-DETR 推理时间      | 备注                   |
| -------------- | -------------------- | ------------------------------------------------- | ------------------------- | --------------------- | ---------------------- |
...
通过使用 [rocm](../configuration/object_detectors.md#amdrocm-gpu检测器) 检测器，Frigate 可以工作在大部分 AMD 的显卡上。
...
单个 MemryX MX3 模块使用默认模型即可处理多路摄像头视频流，足以满足大多数用户需求。对于更大规模的部署（更多摄像头或更大模型），可以使用多个 MX3 模块。Frigate 支持多检测器配置，允许连接多个 MX3 模块来扩展推理能力。

详细信息请参阅[检测器文档](/configuration/object_detectors#memryx-mx3)。

**默认模型配置：**

- 默认模型为 **YOLO-NAS-Small**。

MX3 采用流水线架构，支持的最大帧率（以及支持摄像头数量）不能通过 `1/latency`（1/推理时间）计算，而是单独测量。当估算你的配置可以支持多少路摄像头视频流时，请使用 **MX3 Total FPS** 列来近似估算检测器的限制，而不是推理时间。

| 模型                 | 输入尺寸 | MX3 推理时间 | MX3 总帧率 |
| -------------------- | -------- | ------------ | ---------- |
...
| 平台           | YOLOv9 推理时间 | YOLO-NAS 推理时间           | YOLOx 推理时间          |
| -------------- | --------------- | --------------------------- | ----------------------- |
...
| 模型          | Synaptics SL1680 Inference Time |
| ------------- | ------------------------------- |
| ssd mobilenet | ~ 25 ms                         |
| yolov5m       | ~ 118 ms                        |
