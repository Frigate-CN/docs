...
Frigate 可运行在任何拥有 Docker 的主机上，甚至可以作为[Home Assistant App](https://www.home-assistant.io/apps/)运行。但需要注意的是，Home Assistant App **并不等同于**集成。无论你是将 Frigate 作为独立 Docker 容器运行还是作为 Home Assistant App 运行，都需要通过额外的[集成](/integrations/home-assistant)将 Frigate 接入 Home Assistant 内。
...
| `5000` | 提供给内网的未加密且无需认证的 页面 和 API 访问。该端口可能**造成严重的安全隐患**，使用需要严格限制，仅限 Docker 内部网络中使用，供与 Frigate 集成的服务调用。 |
...
如果你使用 ffmpeg 硬件加速，你**可能**需要使用*完全访问*版本的 App。这是因为 Frigate App 在容器中运行时对主机系统的访问受限。*完全访问*版本允许你禁用*保护模式*，从而让 Frigate 完全访问主机系统。
