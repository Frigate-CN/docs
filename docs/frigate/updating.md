...
Frigate 的当前稳定版本是 **0.17.0**。此版本的发布说明和任何重大变更可以在 [Frigate GitHub 发布页面](https://github.com/blakeblackshear/frigate/releases/tag/v0.16.3) 上找到。

保持 Frigate 的更新可确保你能够获得最新功能、性能改进和错误修复。更新过程根据你的安装方法（Docker、Home Assistant 插件等）略有不同。以下是最常见设置的说明。
...
2. **更新并拉取最新镜像**：
   ...
   - 如果使用 Docker Compose：
     - 编辑你的 `docker-compose.yml` 文件以指定所需的版本标签（例如，使用 `0.17.0` 而不是 `0.16.3`）。例如：
       ```yaml
       services:
         frigate:
...
3. **启动容器**：
   ...
   - 如果使用 Docker Compose：
     ...
     docker compose up -d
     ```
   - 如果使用 `docker run`，使用更新后的镜像标签重新运行你的原始命令（例如，来自 [安装](./installation.md#docker) 部分）。
...
1. **检查更新**：
   ...
   - 找到你安装的 Frigate 插件（例如，"Frigate NVR" 或 "Frigate NVR (Full Access)"）。
...
- 如果使用自定义硬件（例如，Coral 或 GPU），请验证配置仍然有效，因为插件更新不会修改你的硬件设置。
...
2. 恢复你备份的配置文件和数据库。
3. 恢复到之前的镜像版本：
   - 对于 Docker：在你的 `docker run` 命令中指定较旧的标签（例如，`ghcr.io/blakeblackshear/frigate:0.16.3`）。
   - 对于 Docker Compose：编辑你的 `docker-compose.yml`，指定较旧的版本标签（例如，`ghcr.io/blakeblackshear/frigate:0.16.3`），并重新运行 `docker compose up -d`。
