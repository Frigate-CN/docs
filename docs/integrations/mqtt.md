...
为每个变化的追踪目标发布消息。当追踪目标不再被标记为false_positive时发布第一条消息。当Frigate找到追踪目标的更好快照或发生区域变化时，它将发布具有相同id的消息。当追踪目标结束时，将发布一条带有`end_time`设置的最终消息。
...
当追踪目标元数据更新时发布的消息，例如当GenAI运行并返回追踪目标描述时。
...
控制摄像头PTZ自动追踪器的开关主题。期望值为`ON`和`OFF`。

### `frigate/<camera_name>/ptz_autotracker/state`

摄像头PTZ自动追踪器当前状态的主题。发布值为`ON`和`OFF`。

### `frigate/<camera_name>/ptz_autotracker/active`

确定PTZ自动追踪器是否正在主动追踪目标的主题。发布值为`ON`和`OFF`。
