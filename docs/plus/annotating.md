...
为了获得最佳效果，请遵循以下指南。你可能还想查看关于[改进你的模型](./index.md#改进你的模型)的文档。

**标注图像中的每个对象**：在验证之前，标注每张图像中的所有对象非常重要。例如，如果你不标注一辆汽车，模型将被教导图像的那部分**不是**汽车，它将开始变得混乱。你可以排除不希望在任何摄像头上检测到的标签。
...
错误识别的对象应添加正确的标签。例如，如果一个人被错误地检测为猫，你应该在 Frigate 中将其提交为误报，并为该人添加标签。这些框将重叠。
