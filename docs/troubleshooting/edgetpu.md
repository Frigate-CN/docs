...
## USB Coral未被检测到 {#usb-coral-not-detected}
...
### 设备访问权限不正确 {#incorrect-device-access}
...
### Synology 716+II运行DSM 7.2.1-69057 Update 5 {#synology-716ii-running-dsm-721-69057-update-5}
...
### QNAP NAS 设备注意事项 {#qnap-nas}

若你的 QNAP NAS 设备（如 TS-253A）安装了 https://www.qnap.com/en/software/qumagie 及其 QNAP AI Core 扩展组件，系统可能会自动调用连接的 Coral TPU 加速设备。但请注意：当启用 **人脸识别**、**物体识别** 或 **相似照片识别** 任一功能时，Container Station 中运行的 **Frigate** 或 **CodeProject.AI Server** 等应用程序将无法初始化当前使用的 TPU 设备。

如需允许 Coral TPU 设备被正常调用，你必须选择以下任一方案：
...
**重要提示**：完成设置变更后，建议重启你的 NAS 设备以确保配置生效。

## USB Coral检测似乎卡住 {#usb-coral-detection-appears-to-be-stuck}
...
## PCIe Coral未被检测到 {#pcie-coral-not-detected}

PCIe Coral未被检测到最常见的原因是驱动程序未安装。这个过程因操作系统和内核版本而异。

- 在大多数情况下，可以使用https://github.com/jnicolson/gasket-builder 来构建和安装最新版本的驱动程序。

## 尝试加载TPU为pci时出现"Fatal Python error: Illegal instruction" {#attempting-to-load-tpu-as-pci-fatal-python-error-illegal-instruction}

这是由于在新的linux内核上使用过时的gasket驱动程序导致的问题。从https://github.com/jnicolson/gasket-builder 安装更新的驱动程序据报可以解决此问题。

### 在Raspberry Pi5上无法检测 {#not-detected-on-raspberry-pi5}
...
## Coral Dual EdgeTPU只检测到一个PCIe Coral {#only-one-pcie-coral-is-detected-with-coral-dual-edgetpu}
