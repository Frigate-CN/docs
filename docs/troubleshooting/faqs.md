...
Frigate 将每个角色的健康状态发布到 [`frigate/<camera_name>/status/<role>`](/integrations/mqtt#frigatecamera_namestatusrole)，其中 `<role>` 是摄像机上每个已启用的角色（`detect`、`record` 和 `audio`）。发布的值是以下之一：
...
由于状态是按角色区分的，摄像头子码流正常但录像码流失联时，`detect` 会报告 `online`，而 `record` 会报告 `offline`。状态发生变化时会重新发布。
