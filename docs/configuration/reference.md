...
birdseye:
  ...
  #   objects - 如果摄像头在过去30秒内有被追踪的对象，则包含该摄像头
...
detect:
  ...
  fps: 5
  # 可选：对象在追踪器中初始化所需的连续检测命中次数（默认值：帧率的1/2）
  min_initialized: 2
  ...
  stationary:
    ...
    threshold: 50
    # 可选：定义追踪静止对象的最大帧数（默认值：未设置，永久追踪）
    # 这可以帮助减少只应该短时间静止的对象的误报。
    # 也可以用来禁用静止对象追踪。例如，你可能想为person设置一个值，
    # 但让car保持默认值。
    # 警告：设置这些值会覆盖默认行为并禁用静止对象追踪。
    #      很少有情况需要禁用它。除非你知道确实需要，
    #      否则不建议从示例配置中复制这些值到你的配置中。
    max_frames:
      # 可选：所有对象类型的默认值（默认值：未设置，永久追踪）
      default: 3000
      # 可选：特定对象的值
      objects:
        person: 1000
...
record:
  ...
  # 可选：录制持续保留设置
  retain:
    # 可选：无论是否有追踪目标，录像保留的天数（默认值如下所示）
    # 注意：如果您只想保留警报和检测事件的录像，请将此值设为 0，
    #      并在下面的 alerts 和 detections 部分定义保留策略
    days: 0
    # 可选：保留模式。可选值包括：all、motion 和 active_objects
    #   all - 保存所有录像片段，不论是否有活动
    #   motion - 保存所有包含任何运动检测的录像片段
    #   active_objects - 保存所有包含活跃/移动目标的录像片段
    # 注意：此模式仅在上方 days 设置大于 0 时生效
    mode: all
  # 可选：录制导出设置
  export:
...
cameras:
  ...
  back:
    ...
    onvif:
      ...
      # 可选：PTZ摄像头对象自动追踪功能。通过自动移动PTZ摄像头，
      # 将移动对象保持在画面中心。
      autotracking:
        # 可选：启用/禁用对象自动追踪（默认值：如下所示）
        enabled: False
        ...
        # 可选：自动追踪期间对对象进行缩放的模式（默认值：如下所示）
        # 可用选项有：disabled（禁用）、absolute（绝对）和relative（相对）
        #   disabled - 不对自动追踪的对象进行缩放，仅使用平移/倾斜
