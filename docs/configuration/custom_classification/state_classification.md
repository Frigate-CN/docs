  ...
  custom:
    front_door:
      ...
      state_config:
        ...
        cameras:
          front:
            # [x1, y1, x2, y2] 以 0 到 1 之间的小数表示，相对于
            # 摄像头的检测分辨率
            crop: [0.0, 0.25, 0.3, 0.85]
```

裁剪坐标是标准化的（归一化）：每个值是摄像头 `detect` 宽度或高度的比例，而不是像素值。在 UI 向导中绘制裁剪区域会自动生成这些值。
...
### 近期分类记录 {#recent-classifications}

针对状态分类功能，图片仅会在以下特定场景下被存入「近期分类记录」中：

- **首次识别**：摄像头完成的第一次分类识别结果，将永久保存
