...
训练自定义目标分类模型需要联网下载 MobileNetV2 基础权重。默认情况下这些权重不会缓存在 `/config/` 中，因此容器重建后需要重新下载。一旦训练完成，模型将完全离线运行。详见[网络要求](/frigate/network_requirements#one-time-model-downloads)。

:::

## 最低系统要求 {#minimum-system-requirements}

目标分类模型非常轻量级，在 CPU 上运行速度非常快。

模型训练期间，每轮训练大约会短暂占用较高的系统资源，持续 1 到 3 分钟。在性能较低的设备上，训练时间可能会更长。

训练和推理需要 CPU 支持 AVX + AVX2 指令集。
...
### 步骤 1：命名和定义 {#step-1-name-and-define}
...
### 步骤 2：分配训练样本 {#step-2-assign-training-examples}
...
### 提升模型性能 {#improving-the-model}

- **问题定义**：保持类别在视觉上具有明显差异，并与所选目标类型相关。
- **数据收集**：使用模型的“最近分类”选项卡，收集一天中不同时段、不同天气和不同距离下的均衡样本。
- **预处理**：确保示例图像反映与 Frigate 检测框相似的目标裁剪；保持主体居中。
- **裁剪尺寸**：目标裁剪尺寸至少为 100×100 像素（10,000 像素面积）。小于约 80×80 的裁剪在模型 224×224 输入缩放时会被拉伸 3-7 倍，并容易塌缩到特征空间中身份不可靠的通用“团块”区域。如果因为摄像头距离目标较远而导致大多数检测目标较小，考虑重新定位摄像头以获得更近的裁剪。
- **类别平衡**：尽量使最大类别的数量不超过最小类别的约 3 倍。超出此范围，模型会偏向主导类别，并倾向于将边界预测默认归入该类别（即“所有东西看起来都像 Buddy”的失败模式）。
- **阈值**：根据每个模型调整 `threshold` 以减少误分配。从 `0.8` 开始，并根据验证结果进行调整。

:::tip `none` 类别与命名类别的工作方式不同

命名类别最适合使用视觉统一的示例——每张 Buddy 的照片都应该看起来像 Buddy。`none` 类别则需要相反的做法：在不同尺寸、构图和质量上保持视觉多样性，因为在推理时它必须吸收所有不属于你的命名类别的内容。不要将“只保留大型、构图良好的图像”的规则应用于 `none` 类别。刻意混入小裁剪、部分视图和误报——否则模型没有信号来判断“小/模糊的东西 = 不是我的已知类别”，并会默认将这些裁剪强制归入某个命名类别。

:::

## 调试分类模型 {#debugging-classification-models}
