...
要配置加速倍数、帧率等参数，可以使用`timelapse_args`配置参数。以下示例将延时速度改为 60 倍（1 小时录制压缩为 1 分钟），帧率 25FPS：
...
可以使用 ffmpeg 参数`-qp n`（n 代表量化参数值）来减小输出文件大小，调整该值可以在质量和文件大小之间取得平衡。
...
此功能用于修复文件差异，而非完全删除数据库条目。如果你清理了所有监控文件，请务必不要使用`sync_recordings`，而是停止 Frigate 容器，直接删除`frigate.db`数据库后重新启动。
