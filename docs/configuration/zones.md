...
通常你可能希望仅当目标进入关注区域时才创建`警报`。这可以通过设置`required_zones`来实现。例如，仅当目标进入整个院子区域时才创建警报，配置如下：
    ...
    review: # [!code focus]
      ...
      detections: # [!code ++][!code focus]
        required_zones: # [!code ++][!code focus]
          - edge_yard # 区域名称 [!code ++][!code focus]
    zones: # [!code focus]
      edge_yard: # 区域名称 [!code ++][!code focus]
