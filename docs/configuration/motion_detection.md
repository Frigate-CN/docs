...
在调优画面变动检测前，理解目标很重要。在最优配置中，应该能检测到人和车的运动，但不会检测到草叶摇动、光线变化、时间戳等。如果画面变动检测过于敏感，你会遇到更高的CPU负载和更多的误报。如果不够敏感，你会错过想要追踪的对象。
...
观察调试视图中的运动框，调整轮廓面积直到没有比你期望Frigate检测到的最小运动更小的运动框。
