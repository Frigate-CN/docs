...
model:
  ...
  # 必需：物体/目标检测模型输入宽度（默认值：如下所示）
  width: 320
  # 必需：物体/目标检测模型输入高度（默认值：如下所示）
  height: 320
  # 必需：物体/目标检测模型输入颜色空间
...
birdseye:
  ...
  #   objects - 如果摄像头在过去30秒内有被追踪的目标，则包含该摄像头
  #   motion - 如果摄像头在过去30秒内检测到画面变动，则包含该摄像头
  ...
  layout:
    ...
    max_cameras: 1
  # 可选：空闲时（无画面变动或活动更新）重新发送最后合成的鸟瞰图帧的帧率（默认值：如下所示）
...
detect:
  ...
  stationary:
    ...
    # 即便目标检测框的变化幅度达到了判定为画面变动的阈值，依然可精准识别静态目标（默认值：如下所示）
    classifier: True
    ...
    threshold: 50
    # 可选：定义追踪静止目标的最大帧数（默认值：未设置，永久追踪）
    # 这可以帮助减少只应该短时间静止的目标的误报。
    # 也可以用来禁用静止目标追踪。例如，你可能想为person设置一个值，
    # 但让car保持默认值。
    # 警告：设置这些值会覆盖默认行为并禁用静止目标追踪。
    #      很少有情况需要禁用它。除非你知道确实需要，
    #      否则不建议从示例配置中复制这些值到你的配置中。
    max_frames:
      # 可选：所有目标类型的默认值（默认值：未设置，永久追踪）
      default: 3000
      # 可选：特定目标的值
      objects:
        person: 1000
  ...
  # 该值可以是正数或负数。
  # 提示：想象有一个追踪目标剪辑，其中一个人从左向右走。
  #      如果追踪目标生命周期边界框始终在人的左侧，
  #      则应该减小该值。同样，如果一个人从左向右走，
  #      而边界框始终在人的前面，则应该增加该值。
  # 提示：此偏移是动态的，所以你可以更改值，它会更新现有的
  #      追踪目标，这使得调整变得容易。
  # 警告：快速移动的目标的边界框可能无法对齐。
  annotation_offset: 0
...
objects:
  ...
  filters:
    person:    # 人
      ...
      min_area: 5000
      # 可选：检测到的目标边界框的最大尺寸（默认值：24000000）。
      # 可以指定为像素宽度*高度的整数，或表示帧百分比的小数（0.000001到0.99）。
      max_area: 100000
      # 可选：检测到的目标边界框的最小宽高比（默认值：0）
      min_ratio: 0.5
      # 可选：检测到的目标边界框的最大宽高比（默认值：24000000）
      max_ratio: 2.0
      # 可选：开始追踪目标所需的最小分数（默认值：如下所示）
      min_score: 0.5
      # 可选：被追踪目标的计算分数被认为是真阳性的最小百分比（小数）（默认值：如下所示）
      threshold: 0.7
      # 可选：防止在某些区域检测此类型目标的蒙版（默认值：无蒙版）
      # 基于目标边界框的底部中心进行检查
...
review:
  ...
  alerts:
    ...
    labels:
      - car # 汽车
      - person # 人
    # 在无警报触发行为发生后，警报的持续截止时长（默认值：如下所示）
    cutoff_time: 40
  ...
  detections:
    ...
    enabled: True
    # 可选：符合检测条件的标签（默认值：所有被追踪/监听的标签）
    labels:
      - car      # 汽车
      - person   # 人
    # 可选：目标被标记为检测所需的区域（默认值：无）
    # 注意：当在全局设置必需区域时，此区域必须存在于所有摄像头中，
    #      否则配置将被视为无效。在这种情况下，required_zones
    #      应该在摄像头级别配置。
    required_zones:
...
motion:
  ...
  # 注意：画面变动检测是物体/目标检测所必需的，
  #      如果将此设置为False但保持detect启用，
  #      将在启动时导致错误。
  enabled: False
  # 可选：传递给cv2.threshold的阈值，用于确定像素是否足够不同以被计为画面变动（默认值：如下所示）
  # 增加此值将降低画面变动检测的灵敏度，减小此值将提高画面变动检测的灵敏度。
  # 该值应在1到255之间。
  threshold: 30
  # 可选：用于检测闪电或其他需要重新校准画面变动检测的重大变化的图像百分比
  #      （默认值：如下所示）
  # 增加此值将使画面变动检测更可能将闪电或红外模式变化视为有效画面变动。
  # 减小此值将使画面变动检测更可能忽略大量画面变动，例如有人接近门铃摄像头。
  lightning_threshold: 0.8
  # 可选：调整大小后的画面变动图像中被计为画面变动的最小像素尺寸（默认值：如下所示）
  # 增加此值将防止检测到较小的画面变动区域。减小此值将
  # 使画面变动检测对较小的移动物体更敏感。
  # 参考标准：
  #  - 10 - 高灵敏度
  #  - 30 - 中等灵敏度
  ...
  # 较低的值将导致移动阴影等被检测为画面变动的时间更长。
  # https://www.geeksforgeeks.org/background-subtraction-in-an-image-using-concept-of-running-average/
  frame_alpha: 0.01
  # 可选：调整大小后的画面变动帧的高度（默认值：100）
  # 较高的值将以更高的CPU使用为代价获得更细粒度的画面变动检测。
  # 较低的值会减少CPU使用，但小的变化可能不会被识别为画面变动。
  frame_height: 100
  # 可选：画面变动遮罩
  # 注意：有关创建蒙版的更详细信息，请参见文档
  mask: 0.000,0.469,1.000,0.469,1.000,1.000,0.000,1.000
  # 可选：改善对比度（默认值：如下所示）
  # 启用动态对比度改善。这应该有助于改善夜间检测，但代价是使白天的画面变动检测更敏感。
  improve_contrast: True
  # 可选：通过MQTT更新摄像头画面变动状态从ON到OFF时的延迟（默认值：如下所示）。
  mqtt_off_delay: 30
...
record:
  ...
  # 可选：录制持续保留设置
  continuous:
    # 可选：录像文件的强制保留天数（不受识别目标或画面变动检测触发条件限制，默认值：如下所示）
    # 注意：若你仅希望保留由警报和检测事件触发生成的录像文件，需将本参数设置为 0，
    # 并在下方的警报与检测配置段中定义对应的保留规则。
    days: 0
  # 可选: 画面变动保留设置
  motion:
    days: 0
  ...
  alerts:
    ...
    retain:
      ...
      #   motion - 保存所有检测到画面变动的警报录制片段
      #   active_objects - 保存所有有活动/移动目标的警报录制片段
      #
      # 注意：如果摄像头的保留模式比此处配置的模式更严格，
      #      在应用此模式时，片段可能已经被删除。
      #      例如，如果摄像头保留模式是"motion"，没有画面变动的片段
  ...
  detections:
    ...
    retain:
      ...
      #   motion - 保存所有检测到画面变动的检测录制片段
      #   active_objects - 保存所有有活动/移动目标的检测录制片段
      #
      # 注意：如果摄像头的保留模式比此处配置的模式更严格，
      #      在应用此模式时，片段可能已经被删除。
      #      例如，如果摄像头保留模式是"motion"，没有画面变动的片段
...
lpr:
  ...
  model_size: small
  # 可选：开始运行识别所需的车牌目标置信度分数（默认值：如下所示）
  detection_threshold: 0.7
  # 可选：开始运行识别所需的车牌最小面积（默认值：如下所示）
  min_area: 1000
  # 可选：将车牌作为子标签添加到目标所需的识别置信度分数（默认值：如下所示）
  recognition_threshold: 0.9
  # 可选：将车牌作为子标签添加到目标所需的最小字符数（默认值：如下所示）
  min_plate_length: 4
  # 可选：车牌预期格式的正则表达式（默认值：如下所示）
  format: None
  # 可选：允许检测到的车牌与已知车牌匹配时可以有的缺失/错误字符数
  match_distance: 1
  # 可选：要追踪的已知车牌（字符串或正则表达式）（默认值：如下所示）
  known_plates: {}
  ...
  back:
    ...
    zones:
      ...
      front_steps: # 前台阶
        ...
        # 可选：目标被视为在区域内所需的连续帧数（默认值：如下所示）。
        inertia: 3
        # 可选：目标必须在区域内停留的秒数才被视为在区域内（默认值：如下所示）
        loitering_time: 0
    ...
    mqtt:
      ...
      quality: 70
      # 可选：限制MQTT消息仅发送进入所列区域的目标（默认值：无必需区域）
      required_zones: []
    ...
    onvif:
      ...
      # 可选：PTZ摄像头目标自动追踪功能。通过自动移动PTZ摄像头，
      # 将移动目标保持在画面中心。
      autotracking:
        # 可选：启用/禁用目标自动追踪（默认值：如下所示）
        enabled: False
        # 可选：启动时校准摄像头（默认值：如下所示）
        # 校准会逐步移动PTZ并测量移动所需时间。
        # 结果用于帮助估计摄像头移动后追踪目标的位置。
        # Frigate会在校准后自动更新配置文件，
        # 为摄像头添加"movement_weights"条目。之后应将calibrate_on_startup设为False。
        calibrate_on_startup: False
        # 可选：自动追踪期间对目标进行缩放的模式（默认值：如下所示）
        # 可用选项有：disabled（禁用）、absolute（绝对）和relative（相对）
        #   disabled - 不对自动追踪的目标进行缩放，仅使用平移/倾斜
        #   absolute - 使用绝对缩放（大多数PTZ摄像头支持）
        #   relative - 使用相对缩放（并非所有PTZ都支持，但可以实现同步平移/倾斜/缩放运动）
        zooming: disabled
        # 可选：改变自动追踪目标缩放行为的数值（默认值：如下所示）
        # 较低的值会在追踪目标周围保留更多场景。
        # 较高的值会更多地放大追踪目标，但Frigate可能会更快失去追踪。
        # 该值应在0.1到0.75之间
        zoom_factor: 0.3
        # 可选：从labelmap.txt中要追踪的目标列表（默认值：如下所示）
        track:
          - person    # 人
        # 必需：当目标进入任何列出的区域时开始自动追踪
        required_zones:
          - zone_name
        # 必需：追踪结束后返回的ONVIF预设名称（默认值：如下所示）
        return_preset: home
...
telemetry:
  ...
  stats:
    ...
    # 可选：启用摄像头ffmpeg进程、go2rtc和物体/目标检测器的网络带宽统计监控（默认值：如下所示）
