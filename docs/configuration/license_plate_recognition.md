...
- **`device`**: 运行车牌检测和识别模型的设备
  - 默认: `无`
  - 可以让 Frigate 自动选择或手动指定 `CPU`、`GPU` 、GPU 的设备数字 ID。对于没有原生车牌检测模型的用户，使用 GPU 可能提高模型性能，特别是 YOLOv9 车牌检测器模型。然而，对于那些目标检测模型可直接识别车牌（`license_plate`）的用户来说，在 GPU 上运行车牌识别相比在 CPU 上运行，几乎没有性能提升。
