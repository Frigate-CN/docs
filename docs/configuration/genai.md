...
生成式 AI 还可以通过 MQTT 动态切换某个摄像头的启用状态，主题为 `frigate/<camera_name>/object_descriptions/set`。详见 [MQTT 文档](../integrations/mqtt#frigatecamera_nameobjectdescriptionsset)。

## Ollama

:::warning

不建议在 CPU 上使用 Ollama，高推理时间会使生成式 AI 变得不实用。
